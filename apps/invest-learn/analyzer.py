...
def analyze_ticker(ticker: str):
    ...
    for _, row in df.iterrows():
        full_prices = pd.concat(
            [hist, pd.DataFrame([{"price": row["price"]}])], ignore_index=True
        )

        # --- Основные индикаторы ---
        full_prices["sma_5"] = SMAIndicator(
            close=full_prices["price"], window=5
        ).sma_indicator()
        full_prices["sma_20"] = SMAIndicator(
            close=full_prices["price"], window=20
        ).sma_indicator()
        ...
        full_prices["atr"] = AverageTrueRange(
            high=full_prices["price"],
            low=full_prices["price"],
            close=full_prices["price"],
            window=14,
        ).average_true_range()
